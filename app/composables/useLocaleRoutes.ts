...
/**
 * Whether a path owns its locale by virtue of its URL.
 *
 * Only the landing routes do: `/` is English and `/es` is Spanish, and each of
 * those pages pins the shared locale during setup via useLocalePage(). Every
 * other route — the auth and KYC pages of the web app — has no locale in its
 * URL, so it renders in DEFAULT_LOCALE on the server and may adopt the
 * visitor's browser preference once, after hydration. Server output therefore
 * stays deterministic per URL everywhere, with no Accept-Language negotiation
 * anywhere in the app.
 */
export function isLocalePinnedPath(path: string): boolean {
  const normalized = path.replace(/\/+$/, '') || '/'
  return (Object.values(LOCALE_PATH) as string[]).includes(normalized)
}

/**
 * The visitor's preferred locale, if we support it. Client-side only: this
 * reads `navigator.language`, never a request header.
 */
