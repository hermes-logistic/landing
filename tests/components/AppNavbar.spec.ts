...
// The drawer's opaque panel: second child of #mobile-menu, after the scrim.
// Shared by the sign-up link block and the drawer contract block below.
...
describe('AppNavbar', () => {
  ...
  // The route-level entry point into the web app. develop's navbar carried a
  // /signup link that this rebuild has to keep; DESIGN.md
  // `components.navbar-signup-link` fixes what it looks like and where it sits.
  describe('sign-up link', () => {
    const signupLinks = (wrapper: ReturnType<typeof mountNavbar>) =>
      wrapper.findAll('a[href="/signup"]')

    it('renders a crawlable link to /signup on the bar', () => {
      const wrapper = mountNavbar()
      const bar = signupLinks(wrapper)
      expect(bar).toHaveLength(1)
      expect(bar[0]!.text()).toBe('nav.signup')
    })

    it('is not a fifth entry in NAV_LINKS', () => {
      const wrapper = mountNavbar()
      // The four anchors are the scroll-spy list; a route does not belong there.
      expect(wrapper.findComponent(NavLinks).findAll('a[href="/signup"]')).toHaveLength(0)
      expect(NAV_LINKS.some(l => l.href === '/signup')).toBe(false)
    })

    it('carries no aria-current: it points at another document', () => {
      const wrapper = mountNavbar()
      expect(signupLinks(wrapper)[0]!.attributes('aria-current')).toBeUndefined()
    })

    it('wears the nav-link tokens, not the CTA pill', () => {
      const link = signupLinks(mountNavbar())[0]!
      expect(link.classes()).toEqual(expect.arrayContaining([
        'text-[#EBF2FF]',
        'hover:text-[#61F0FF]',
        'font-medium',
      ]))
      // No fill, no border, no radius — the bar carries exactly one CTA.
      for (const c of ['rounded-full', 'border', 'border-[#FF734D]']) {
        expect(link.classes()).not.toContain(c)
      }
    })

    it('stays off the bar below 1280, where no Spanish label fits', () => {
      // barVisibleFrom: 1280px, a measured value — see DESIGN.md "Sign-up
      // entry point". `hidden` also drops it out of the cluster's flex gap, so
      // below xl it costs the row no width at all.
      const link = signupLinks(mountNavbar())[0]!
      expect(link.classes()).toEqual(expect.arrayContaining(['hidden', 'xl:inline-flex']))
    })

    it('appears in the drawer action group above the CTA and closes it on click', async () => {
      const wrapper = mountNavbar()
      await openDrawer(wrapper)

      const actionGroup = panel(wrapper).findAll(':scope > div')[1]!
      const rows = actionGroup.findAll('a, button')
      // Both rows are nav-md, so order ranks them: the primary goes last.
      expect(rows.map(r => r.text())).toEqual(['nav.signup', 'nav.cta'])
      // itemGap {spacing.4} between the two, and the 44px touch floor on the link.
      expect(actionGroup.classes()).toContain('gap-1')
      expect(rows[0]!.classes()).toEqual(expect.arrayContaining(['min-h-[44px]', 'text-sm']))

      await rows[0]!.trigger('click')
      expect(drawer(wrapper).props('open')).toBe(false)
    })

    it('keeps the drawer at three groups', async () => {
      const wrapper = mountNavbar()
      await openDrawer(wrapper)
      // A fourth group would need a fourth hairline and a type step below
      // nav-sm, which does not exist. The action group grew instead.
      expect(panel(wrapper).findAll(':scope > div')).toHaveLength(3)
    })
  })
